...
구조는 크게 세 부분으로 나눌 수 있다. 먼저 `answer` 배열을 `commands.length` 크기로 미리 만들어서, 명령 개수만큼 결과를 담을 자리를 확보했다. 그다음 `for` 문으로 commands를 순회하면서 각 명령에서 `i, j, k` 값을 꺼내고, `copyOfRange`로 자른 뒤 `sort`로 정렬하고, 정렬된 배열의 `k - 1`번째 값을 `answer[idx]`에 저장했다. 마지막으로 모든 명령을 처리한 뒤 `answer`를 반환하는 구조다.
